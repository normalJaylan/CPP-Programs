...
public :
	Prime(){};
	string get_primenumber();    // n表示要生成的随机数的位数
...
string Prime::get_primenumber()   // 使用当前时钟作为随机数种子
{
	char p[51];
	
	srand((unsigned)time(NULL));  
	
	readfile();							// 将10000以内的素数读入prime数组
...
bool Prime::division(string p)		// 判断该数是否能被10000以内的素数整除
{	
...
string Prime::div(string str1, string str2, int flag) {//高精度除法
...
string Prime::add(string str1,string str2) {//高精度加法
...
string Prime::mul(string str1,string str2) {//高精度乘法
    int sign=1; //sign 为符号位
    string str;
    if (str1[0]=='-') {
        sign*=-1;
...
string Prime::sub(string str1,string str2) {//高精度减法
    int sign=1; //sign 为符号位
    string str;
    int i,j,k;
	char temp[100];
	if (str1[0] == '-' && str2[0] != '-')	// 如果是负数减正数，减数取反后算加法
	{
		temp[0] = '-';
...
void Prime::readfile()					// 将10000以内的素数读入prime数组
{
...
int Prime::compare(string str1, string str2)			//相等返回0，大于返回1，小于返回-1
{
